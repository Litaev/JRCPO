...
void Car::setCar(QString newCarName, QString newCarYear, QString newCarMileage, QString newCarFuelType, QString newCarTankVolume, QString newCarFilepath) {
    carName = newCarName;
    ...
    carImageFilePath = newCarFilepath;
...
CustomContainer<std::shared_ptr<Event>>& Car::getEventList(){
    return eventList;
}
...
QJsonObject Car::toJson(int chosenEventType, int chosenEventMinPrice) const {
    QJsonObject carObj;
    carObj["carName"] = carName;
    carObj["carImageFilePath"] = carImageFilePath;
    carObj["carGovNumber"] = carGovNumber;
    carObj["carYear"] = carYear;
    carObj["carMileage"] = carMileage;
    carObj["carTankVolume"] = carTankVolume;
    carObj["carFuelType"] = static_cast<int>(carFuelType);

    QJsonArray eventsArray;
    for (const auto &event : eventList) {
        if (chosenEventType == -1){
            if (chosenEventMinPrice <= event->getEventPrice()){
                eventsArray.append(event->toJson());
            }
        }
        else if (chosenEventType == static_cast<int>(event->getEventType())){
            if (chosenEventMinPrice <= event->getEventPrice()){
                eventsArray.append(event->toJson());
            }
        }
