...
Item {
    ...
    property string carImageFilepath: {
        if (status.isEditCar == true){
            user.getInfoAboutCar(user.getUserChosenCarId(), "imagePath")
        }
        else{
            ""
        }
    ...
    function confirmAction(){
        if (isCarNameGood + isCarYearGood + isCarMileageGood + isCarTankVolumeGood == 0){
            if (status.isEditCar == false){
                user.addCar(carNameTextField.text, carYearTextField.text, carMileageTextField.text, carFuelTypeComboBox.currentIndex, carTankTextField.text, carImageFilepath);
                chooseCarWindow.updateModel();
            }
            else{
                user.editChosenCarInfo(carNameTextField.text, carYearTextField.text, carMileageTextField.text, carFuelTypeComboBox.currentIndex, carTankTextField.text, carImageFilepath);
                chooseCarWindow.updateModel();
                _loader.reload();
            }
            stackView.pop();
    ...
    Rectangle{
        ...
        Button{
            ...
            anchors.margins: 10
            onClicked: {
                carWindowItem.carImageFilepath = user.getImagePath();
            }
            background: Image {
                sourceSize: Qt.size(width, width)
                fillMode: Image.PreserveAspectCrop
                source: carWindowItem.carImageFilepath == "" ? "qrc:/test11/icons/car_white_icon.png" : carWindowItem.carImageFilepath
            }
        }
    }
    Rectangle{
        ...
        anchors.top: carRect.bottom
        anchors.topMargin: 5
        anchors.left: carImageRect.right
        anchors.leftMargin: 5
        anchors.right: parent.right
        anchors.rightMargin: 5
        height: (carImageRect.width/2.5)
        color: "#6C757D"
        radius: 10
        Image {
            id: carYearImage
    ...
    Rectangle{
        ...
        anchors.right: parent.right
        anchors.rightMargin: 5
        anchors.bottom: carImageRect.bottom
        height: carYearRect.height
        color: "#495057"
        radius: 10
        Image {
            id: carMileageImage
    ...
    Rectangle{
        id: carNameRect
        anchors.top: carImageRect.bottom
        anchors.topMargin: 5
        anchors.left: parent.left
        anchors.leftMargin: 5
        anchors.right: parent.right
